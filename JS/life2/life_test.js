    ...
    new Test.Unit.Runner({
		...
		Object.extend(this, {
			assertNext: function(grid, coords) {
			    ...
			    assertOldAge: function(grid,generationsAfterOldAge){
			    for(var i=0;i<=oldAge;i++,grid=grid.evolve());
			    for(var i=0;i<generationsAfterOldAge.length;i++,grid=grid.evolve())
				this.assertLiveCells(grid.live_cells(),generationsAfterOldAge[i]);
			    },
		...
		test_live_coords:function() {
		with (this) {
		    var coords = $A([new CoOrd(1, 1),
				     new CoOrd(1, 0),
				     new CoOrd(1, 2)]);
		    var live_coords = make_grid(bound,coords).live_coords();
		    coords.each(function (coord){assertInList(live_coords,coord);});
		}
		},	
		...
		test_blinker_old_age:function() {
		with (this) {
		    assertOldAge(make_grid(bound,
					 $A([new CoOrd(1, 1),
					     new CoOrd(1, 0),
					     new CoOrd(1, 2)])
					 ),
		...
		with (this) {
		    var stableBlock = $A([new CoOrd(1, 1),
					  ...
					  new CoOrd(2, 1),
					  new CoOrd(2, 2)]);
		    assertNext(make_grid(bound,stableBlock),stableBlock);
			}

	    },
		test_stable_block_old_age:function() {
		with (this) {
		    assertOldAge(make_grid(bound,$A([new CoOrd(1, 1),
						     new CoOrd(1, 2),
						     new CoOrd(2, 1),
						     new CoOrd(2, 2)])),
				 [[]]);
		    assertOldAge(make_grid(bound,$A([new CoOrd(0, 1),
						     new CoOrd(1, 0),
						     new CoOrd(2, 1),
						     new CoOrd(0, 2),
						     new CoOrd(1, 2)])),
		...
		var phases = $A([$A([
				 ...
				 $A([new CoOrd(0, 2),
				     ...
				     new CoOrd(2, 4),
				     new CoOrd(3, 3)])]);
		var grid = make_grid(this.bound, phases[0]);
		for (var i = 0; i < 2; i++) {
		    grid = grid.evolve();
		    this.assertLiveCells(grid.live_cells(), phases[(i + 1) % 2]);
		}
	    },
		test_toad_old_age:function(){
		with (this) {
		    assertOldAge(make_grid(bound,
					 $A([new CoOrd(1, 1),
					     new CoOrd(1, 2),
...
function test_cell(log) {
    new Test.Unit.Runner({
		...
		with (this) {
		    cell = new Cell(alive);
		    for(var i=0;i<3;i++) {
			assertEquals(i,cell.generation);
			cell.update();
		    }
		}
	    },
		test_clone:function(){
		    with (this) {
			cell = new Cell(alive,2);
			add_live_neighbours(2);
			var clone = cell.clone();
			assertEquals(cell.generation,clone.generation);
			assertEquals(cell.state,clone.state);
			assert(2==cell.live_neighbours);
			assert(0==clone.live_neighbours);
		    }
		},
		test_death_by_ageing:function(){
		    with(this){
			this.cell = new Cell(alive);
			add_2live_neighbours();
			while(cell.generation<3) {
			    assertAlive(); 
			    cell.update();
			}
			assertDead();
