...
(define tests
  (test-suite
   ...
   (check-equal? (stream-for-n-steps (stream-add-zero funny-number-stream) 5) 
                 (list (cons 0 1) (cons 0 2) (cons 0 3) (cons 0 4) (cons 0 -5)) "stream-add-zero test")
   
   ; cycle-lists test
   (check-equal? (stream-for-n-steps (cycle-lists (list 1 2 3) (list "a" "b")) 8) (list (cons 1 "a") (cons 2 "b") (cons 3 "a") 
                                                                                        (cons 1 "b") (cons 2 "a") (cons 3 "b") 
                                                                                        (cons 1 "a") (cons 2 "b")) "cycle-lists test")
   
   ; vector-assoc test
   (check-equal? (vector-assoc 4 (vector 4 (cons 2 1) "foo" (cons 3 1) (cons 4 1) (cons 5 1))) (cons 4 1) "vector-assoc test")
